...
// Write code to detect collision between player arrow and opponent
  ...
  for(var i = 0; i <playerArrows.length; i++ ){
    ...
    var bowCollision = Matter.SAT.collides(playerArrows[i].body, computerArcher.body);

    var computerCollision = Matter.SAT.collides(playerArrows[i].body, computer.body);

    if(baseCollision.collided || bowCollision.collided || computerCollision.collided){
      console.log("Player Arrow Collision");
      fill("#FFFF");
      textAlign("center");
      textSize(40);
      text("Player Arrow Collsion", width / 4, 100);
    }
  }
}
...
function handleComputerArrowCollision() {
  ...
  for(var i = 0; i <computerArrows.length; i++ ){
    ...
    var bowCollision = Matter.SAT.collides(computerArrows[i].body, playerArcher.body);

    var playerCollision = Matter.SAT.collides(computerArrows[i].body, player.body);

    if(baseCollision.collided || bowCollision.collided || playerCollision.collided){
      console.log("Computer Arrow Collision");
      fill("#FFFF");
      textAlign("center");
      textSize(40);
      text("Player Arrow Collsion", width - 200, 100);
    }
  }
}
